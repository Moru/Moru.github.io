...
TS.pl_2 = TS.pl_2 || {};
TS.pl_2.adsRenderedInSlot = 0;
TS.pl_2.currentSlot = (TS.pl_2.currentSlot === undefined) ? 1 : TS.pl_2.currentSlot + 1;
TS.pl_2.adsRenderedInSpecificSlot = TS.pl_2.adsRenderedInSpecificSlot || {};
TS.pl_2.adsRenderedInSpecificSlot[TS.pl_2.currentSlot] = 0;
TS.pl_2.maxAds = 1;
TS.pl_2.placementType=0;
//false
TS.pl_2.adHeaderEnabled=false;
TS.pl_2.placementStr="midContent";
}(TS = window.TS || {}));
var G = window;
...
G.tsAdsRendered_midContent_slot[TS.pl_2.currentSlot-1]=0;
if(G.demographRendered === undefined){ G.demographRendered = false; }
if(G.ts_pl_2_idx === undefined){G.ts_pl_2_idx = 1;}
...
++TS.pl_2.adsRenderedInSlot;
++TS.pl_2.adsRenderedInSpecificSlot[TS.pl_2.currentSlot];
TS.log("2 - Ads rendered in slot "+slot + "=" + TS.pl_2.adsRenderedInSpecificSlot[TS.pl_2.currentSlot]);
};
...
(function(TS) {
TS.createFillerContainer2 = function() {
if(TS.pl_2.fillerCarousel === undefined)
{
TS.pl_2.fillerCarousel = {};
TS.pl_2.fillerCarousel.fillerAds = [];
TS.pl_2.fillerCarousel.mobileFillerAds = [];
TS.pl_2.fillerCarousel.tabletFillerAds = [];
TS.pl_2.fillerCarousel.fillerAdSlots = [];
TS.pl_2.fillerCarousel.slotSpecificFiller = [];
TS.pl_2.fillerCarousel.slotSpecificMobileFiller = [];
TS.pl_2.fillerCarousel.slotSpecificTabletFiller = [];
TS.pl_2.fillerCarousel.started = false;
TS.pl_2.fillerCarousel.chosenFillerMap = {};
}
return TS.pl_2.fillerCarousel;
};
TS.addFillerSlot2 = function(fillerCarousel) {
var id = "ts-filler-2-" + TS.pl_2.currentSlot;
fillerCarousel.fillerAdSlots.push(
{
"id": id,
"fillerAd":undefined,
"currentSlot": TS.pl_2.currentSlot
}
);
...
TS.getFillerBySpecificSlot2 = function(fillerAds, slot, fillerNr) {
var i;
...
TS.getRandomFillerWithHighestPrio2 = function(fillerAds) {
...
TS.fillerCarousel2 = function(fillerHolder) {
if(fillerHolder.started === false)
{
TS.log("pl_2 - starting carousel");
...
{
TS.log("pl_2 - No fillers");
clearInterval(renderAllFillers);
}
else if(maxTime === 0)
{
TS.log("pl_2 - Time is up");
clearInterval(renderAllFillers);
}
else
{
...
{
TS.log("pl_2 - Num filler slots "+fillerHolder.fillerAdSlots.length);
var slot = fillerHolder.fillerAdSlots[0];
if(TS.pl_2.adsRenderedInSpecificSlot[slot.currentSlot] > 0)
{
fillerHolder.fillerAdSlots.splice(0, 1);
TS.log("pl_2 - already rendered an ad in slot "+slot.currentSlot);
return;
}
...
{
TS.log("pl_2 - No ads rendered in slot "+slot.currentSlot + " yet, proceed")
}
...
var fillerNr = TS.getRandomFillerWithHighestPrio2(fillerHolder.mobileFillerAds);
while(TS.getFillerBySpecificSlot2(fillerHolder.mobileFillerAds, slot, fillerNr) === false) {
fillerNr = TS.getRandomFillerWithHighestPrio2(fillerHolder.mobileFillerAds);
}
...
var fillerNr = TS.getRandomFillerWithHighestPrio2(fillerHolder.tabletFillerAds);
while(TS.getFillerBySpecificSlot2(fillerHolder.tabletFillerAds, slot, fillerNr) === false) {
fillerNr = TS.getRandomFillerWithHighestPrio2(fillerHolder.tabletFillerAds);
}
...
var fillerNr = TS.getRandomFillerWithHighestPrio2(fillerHolder.fillerAds);
while(TS.getFillerBySpecificSlot2(fillerHolder.fillerAds, slot, fillerNr) === false) {
fillerNr = TS.getRandomFillerWithHighestPrio2(fillerHolder.fillerAds);
}
...
TS.log("pl_2 - No chosen filler for slot "+slot.id);
return;
}
if(chosenFiller.inProcess)
{
TS.log("pl_2 - Impossible state for filler "+chosenFiller.id + ", num ads rendered in slot "+slot.currentSlot)
}
if (!TS.renderedFillersPlacement2) {
TS.renderedFillersPlacement2 = [];
}
var fillerAlreadyRendered2 = false;
for (var i = 0; i < TS.renderedFillersPlacement2.length; i++) {
if (TS.renderedFillersPlacement2[i] === chosenFiller.id) {
fillerAlreadyRendered2 = true;
}
}
chosenFiller.inProcess = true;
TS.pl_2.fillerCarousel.chosenFillerMap[chosenFiller.id] = chosenFiller;
TS.log("pl_2 - Processing filler "+chosenFiller.id + ", " + chosenFiller.name + ", num ads rendered in slot "+slot.currentSlot + " = "+TS.pl_2.adsRenderedInSpecificSlot[slot.currentSlot]);
var slotElement = document.getElementById(slot.id);
TS.log("pl_2 - Got a slot for "+chosenFiller.id + ", div: "+slot.id);
if(slotElement && slotElement !== undefined && !fillerAlreadyRendered2)
{
TS.renderedFillersPlacement2.push(chosenFiller.id);
...
"window.parent.TS.passback2(" + chosenFiller.id+", '"+slot.id +"');"+
"}" +
"</scr"+"ipt>";
...
TS.log("2 - Appending "+chosenFiller.name+" to slot "+slot.id +"\n"+html+"\n");
...
--max;
var checkRes = TS.checkFillerAdRendered2(startTime, max, chosenFiller, slot, iframe, fillerHolder);
...
{
TS.log("pl_2 - Slot "+slot.currentSlot + " already is processing "+slot.fillerAd.id + ", "+slot.fillerAd.name)
}
}
}
}, 100);
...
TS.passback2 = function(fillerId, slotId)
{
var chosenFiller = TS.pl_2.fillerCarousel.chosenFillerMap[fillerId];
TS.log(slotId + " - Passback called for "+ chosenFiller.name +", aborting");
chosenFiller.inProcess = false;
};
TS.checkFillerAdRendered2 = function(startTime, max, chosenFiller, slot, iframe, fillerHolder){
var slotElement = document.getElementById(slot.id);
...
TS.log("pl_2 - img css height: "+cssH);
if(cssH > newheight)
{
newheight = cssH;
}
...
TS.log("pl_2 - img css width: "+cssW);
if(cssW > newwidth)
{
newwidth = cssW;
}
...
TS.log("pl_2 - object height: "+h);
if(h > newheight)
{
newheight = h;
}
var w = objects[i].width;
TS.log("pl_2 - object width: "+w);
if(w > newwidth)
{
newwidth = w;
}
...
TS.log("pl_2 - iframe height: "+h);
if(h > newheight)
{
newheight = h;
}
var w = iframes[i].width;
TS.log("pl_2 - iframe width: "+w);
if(w > newwidth)
{
newwidth = w;
}
...
TS.log("pl_2 - Setting height of slot "+slot.id + " "+ chosenFiller.name +" to "+newheight + ", time left: "+(max/10) + "s");
if(max === 0)
{
TS.log("pl_2 - " + slot.id + " "+ chosenFiller.name + " cancelling");
slot.fillerAd = undefined;
return true;
}
else if (newheight > 50 || ((slotHeight > 50 || (TS.isMobileDevice() && slotHeight > 40)) && !isKeyM))
{
TS.log("pl_2 - something rendered in " + slot.id + " "+chosenFiller.name);
for(var j = 0; j< fillerHolder.fillerAdSlots.length; j++)
{
if(fillerHolder.fillerAdSlots[j].currentSlot == slot.currentSlot)
{
TS.log("pl_2 - splicing slot "+j + " for slot "+slot.id + " "+chosenFiller.name);
...
"&ttr="+renderTime;
TS.sendStats(url);
++TS.pl_2.adsRenderedInSpecificSlot[slot.currentSlot];
return true;
}
else
{
...
TS.fillerAdsDisabled2 = function(placementId, fillerAds){
var fillerDisabled = typeof G.tsFillerDisabledSlots_midContent === "object" &&
G.tsFillerDisabledSlots_midContent.length &&
TS.contains(G.tsFillerDisabledSlots_midContent, TS['pl_' + placementId].currentSlot);
return fillerDisabled;
};
TS.processFillerAds2 = function(placementId, fillerAds){
var fillerDisabled = typeof G.tsFillerDisabledSlots_midContent === "object" && G.tsFillerDisabledSlots_midContent.length && TS.contains(G.tsFillerDisabledSlots_midContent, TS['pl_' + placementId].currentSlot);
...
{
TS.log("pl_2 - Fillers disabled / no fillers");
}
};
TS.renderFillerAd2 = function (currentSlot, fillerAds, placementId) {
if(fillerAds.length > 0) {
var id = "ts-fillers-"+currentSlot+"-"+placementId;
var randomFiller = 0;             var chosenFiller = fillerAds[randomFiller];
TS.log("pl_2 - Rendering filler supplier = " + chosenFiller.fillerSupplier);
...
var fillerCarousel2 = TS.createFillerContainer2();
TS.pl_2.fillerCarousel=fillerCarousel2;
TS.addFillerSlot2(fillerCarousel2);
/*Num Fillers: 4*/
/*Num mobile Fillers: 2 */
/*Num mobile Fillers: 1 */
//[[com.tailsweep.core.model.dto.SitePlacementFillerSupplierDto@3502ca8, com.tailsweep.core.model.dto.SitePlacementFillerSupplierDto@4a3cb4b], [com.tailsweep.core.model.dto.SitePlacementFillerSupplierDto@1d77ab4e], [com.tailsweep.core.model.dto.SitePlacementFillerSupplierDto@1b8f9cff], [com.tailsweep.core.model.dto.SitePlacementFillerSupplierDto@718c377b]]
if(!TS.fillerAdsDisabled2()){
var fillerCarousel2 = TS.createFillerContainer2();
TS.pl_2.fillerCarousel=fillerCarousel2;
if( TS.isMobileDevice() ) {
if(fillerCarousel2.mobileFillerAds.length == 0)
{
var tempA = [];
TS.log("pl_2  - Adding mobile fillersupplier id: 22124, name: BT Fusion TS Mobil Mid Mat & Dryck, placement: 2, waterfall: 1")
tempA.push({
"id" : 22124,
"sitePlacement" : 5284660,
"fillerSupplier" : 684,
"content" : "<"+"!-- TailSweep, FusionSpotTag-Mobile -"+"-> <scr"+"ipt> window.frameElement.style.height = \"320px\"; window.frameElement.style.width = \"320px\"; Fuspartnetwork = \"Tailsweep\"; Fusparttsbid = window.top.tsId; Fusparttsdomain = window.location.hostname; Fusparttsbvertical = \"Mat o Dryck\"; Fusadserver = \"fusion.bonniertidskrifter.se\"; Fusmediazone = \"bt.mobile.tailsweep.mat__dryck.ros\"; Fussinglespace = \"tailsweep_mobile_mid\"; Fusparameters = \"tsbnetwork=\" + Fuspartnetwork + \"&tsbid=\" + Fusparttsbid + \"&tsbdomain=\" + Fusparttsdomain + \"&tsbvertical=\" + Fusparttsbvertical; <\/script> <scr"+"ipt src=\"http:\/\/fusion.bonniertidskrifter.se\/redirscrpt.js\"><\/script> ",
"currentSlot" : TS.pl_2.currentSlot,
"inProcess":false,
"name":"BT Fusion TS Mobil Mid Mat & Dryck",
"waterfallLevel" : 1,
"timeToLive":1,
"slotSpecific" : []
});
TS.log("pl_2  - Adding mobile fillersupplier id: 22455, name: Bt Fusion TS Mobil Mid Kvinna & Livsstil , placement: 2, waterfall: 3")
tempA.push({
"id" : 22455,
"sitePlacement" : 5284660,
"fillerSupplier" : 688,
"content" : "<"+"!-- TailSweep, FusionSpotTag-Mobile -"+"-> <scr"+"ipt> Fuspartnetwork = \"Tailsweep\"; Fusparttsbid = window.top.tsId; Fusparttsdomain = window.location.hostname; Fusparttsbvertical = \"Kvinna o Livsstil\"; Fusadserver = \"fusion.bonniertidskrifter.se\"; Fusmediazone = \"bt.mobile.tailsweep.kvinna__livsstil.ros\"; Fussinglespace = \"tailsweep_mobile_mid\"; Fusparameters = \"tsbnetwork=\" + Fuspartnetwork + \"&tsbid=\" + Fusparttsbid + \"&tsbdomain=\" + Fusparttsdomain + \"&tsbvertical=\" + Fusparttsbvertical; <\/script> <scr"+"ipt src=\"http:\/\/fusion.bonniertidskrifter.se\/redirscrpt.js\"><\/script> ",
"currentSlot" : TS.pl_2.currentSlot,
"inProcess":false,
"name":"Bt Fusion TS Mobil Mid Kvinna & Livsstil ",
"waterfallLevel" : 3,
"timeToLive":1,
"slotSpecific" : []
});
fillerCarousel2.mobileFillerAds.push.apply(fillerCarousel2.mobileFillerAds, tempA);
var tempA = [];
TS.log("pl_2  - Adding mobile fillersupplier id: 21172, name: Yieldoptimisers Mobile Shuffle Mat & Dryck, placement: 2, waterfall: 10")
tempA.push({
"id" : 21172,
"sitePlacement" : 5284660,
"fillerSupplier" : 467,
"content" : "<"+"!-- Ad Unit [2104] Tailsweep - RON - Mat och Dryck - Mobile Shuffle 300x250 \/ 320x320 \/ 640x640 -"+"-><div id=\"Yield MS\" style=\"width: 320px; height: 320px; left: 0px; top: 0px;\"><div style=\"position: relative; width: 100%; height: 100%; overflow: hidden;\"> <div id=\"switch_placeholder_1576a91d29f8368848099afd2ec3210d\" class=\"switch_placeholder\"><\/div> <scr"+"ipt> (__scads = window.__scads || []).push({\"z\":2104,\"targetId\":\"switch_placeholder_1576a91d29f8368848099afd2ec3210d\",\"domain\":\"delivery.yieldoptimisers.net\",\"width\":\"320\",\"height\":\"320\"}); <\/script> <scr"+"ipt async src=\"\/\/delivery.yieldoptimisers.net\/adserver\/sat.js\"><\/script><\/div><\/div> ",
"currentSlot" : TS.pl_2.currentSlot,
"inProcess":false,
"name":"Yieldoptimisers Mobile Shuffle Mat & Dryck",
"waterfallLevel" : 10,
"timeToLive":1,
"slotSpecific" : []
});
...
if (TS.isTablet()) {
if (fillerCarousel2.tabletFillerAds.length == 0)
{
var tempA = [];
TS.log("pl_2  - Adding tablet fillersupplier id: 21174, name: Yieldoptimisers Tablet Panorama Shuffle Mat och Dryck, placement: 2, waterfall: 10")
tempA.push({
"id" : 21174,
"sitePlacement" : 5284660,
"fillerSupplier" : 615,
"content" : "<"+"!-- Ad Unit [2107] Tailsweep - RON - Mat och Dryck - Panorama Shuffle 728x90 \/ 980x120 \/ 980x240 \/ 980x300 \/ 970x90 \/ 970x250 -"+"-><div id=\"Yield PS\" style=\"width: 980px; height: 300px; left: 0px; top: 0px;\"><div style=\"position: relative; width: 100%; height: 100%; overflow: hidden;\"> <div id=\"switch_placeholder_9d338beeeb3536880e3f25c76c70c893\" class=\"switch_placeholder\"><\/div> <scr"+"ipt> (__scads = window.__scads || []).push({\"z\":2107,\"targetId\":\"switch_placeholder_9d338beeeb3536880e3f25c76c70c893\",\"domain\":\"delivery.yieldoptimisers.net\",\"width\":\"980\",\"height\":\"300\"}); <\/script> <scr"+"ipt async src=\"\/\/delivery.yieldoptimisers.net\/adserver\/sat.js\"><\/script><\/div><\/div> ",
"currentSlot" : TS.pl_2.currentSlot,
"inProcess":false,
"name":"Yieldoptimisers Tablet Panorama Shuffle Mat och Dryck",
"waterfallLevel" : 10,
"timeToLive":1,
"slotSpecific" : []
});
fillerCarousel2.tabletFillerAds.push.apply(fillerCarousel2.tabletFillerAds, tempA);
}
}
if (!TS.isMobileDevice() && !TS.isTablet()) {
if(fillerCarousel2.fillerAds.length == 0)
{
var tempA = [];
TS.log("pl_2  - Adding fillersupplier id: 22123, name: BT Fusion TS Desktop Mid Mat & Dryck, placement: 2, waterfall: 1");
tempA.push({
"id" : 22123,
"sitePlacement" : 5284660,
"fillerSupplier" : 683,
"content" : "<scr"+"ipt> window.frameElement.style.height = \"300px\"; window.frameElement.style.width = \"980px\"; Fuspartnetwork = \"Tailsweep\"; Fusparttsbid = window.top.tsId; Fusparttsdomain = window.location.hostname; Fusparttsbvertical = \"Mat o Dryck\"; Fusadserver = \"fusion.bonniertidskrifter.se\"; Fusmediazone = \"bt.desktop.tailsweep.mat__dryck.ros\"; Fussinglespace = \"tailsweep_desktop_mid\"; Fusparameters = \"tsbnetwork=\" + Fuspartnetwork + \"&tsbid=\" + Fusparttsbid + \"&tsbdomain=\" + Fusparttsdomain + \"&tsbvertical=\" + Fusparttsbvertical; <\/script> <scr"+"ipt src=\"http:\/\/fusion.bonniertidskrifter.se\/redirscrpt.js\"><\/script> ",
"currentSlot" : TS.pl_2.currentSlot,
"inProcess":false,
"name":"BT Fusion TS Desktop Mid Mat & Dryck",
"waterfallLevel" : 1,
"timeToLive":1,
"slotSpecific" : []
});
TS.log("pl_2  - Adding fillersupplier id: 23854, name: BT Fusion TS Desktop Mid Mode & Skönhet , placement: 2, waterfall: 2");
tempA.push({
"id" : 23854,
"sitePlacement" : 5284660,
"fillerSupplier" : 689,
"content" : "<scr"+"ipt> Fuspartnetwork = \"Tailsweep\"; Fusparttsbid = window.top.tsId; Fusparttsdomain = window.location.hostname; Fusparttsbvertical = \"Mode o Sk\u00F6nhet\"; Fusadserver = \"fusion.bonniertidskrifter.se\"; Fusmediazone = \"bt.desktop.tailsweep.mode__skonhet.ros\"; Fussinglespace = \"tailsweep_desktop_mid\"; Fusparameters = \"tsbnetwork=\" + Fuspartnetwork + \"&tsbid=\" + Fusparttsbid + \"&tsbdomain=\" + Fusparttsdomain + \"&tsbvertical=\" + Fusparttsbvertical; window.frameElement.style.height = \"300px\"; window.frameElement.style.width = \"980px\"; <\/script> <scr"+"ipt src=\"http:\/\/fusion.bonniertidskrifter.se\/redirscrpt.js\"><\/script> ",
"currentSlot" : TS.pl_2.currentSlot,
"inProcess":false,
"name":"BT Fusion TS Desktop Mid Mode & Skönhet ",
"waterfallLevel" : 2,
"timeToLive":1,
"slotSpecific" : []
});
fillerCarousel2.fillerAds.push.apply(fillerCarousel2.fillerAds, tempA);
var tempA = [];
TS.log("pl_2  - Adding fillersupplier id: 22456, name: BT Fusion TS Desktop Mid Kvinna & Livsstil , placement: 2, waterfall: 3");
tempA.push({
"id" : 22456,
"sitePlacement" : 5284660,
"fillerSupplier" : 687,
"content" : "<scr"+"ipt> Fuspartnetwork = \"Tailsweep\"; Fusparttsbid = window.top.tsId; Fusparttsdomain = window.location.hostname; Fusparttsbvertical = \"Kvinna o Livsstil\"; Fusadserver = \"fusion.bonniertidskrifter.se\"; Fusmediazone = \"bt.desktop.tailsweep.kvinna__livsstil.ros\"; Fussinglespace = \"tailsweep_desktop_mid\"; Fusparameters = \"tsbnetwork=\" + Fuspartnetwork + \"&tsbid=\" + Fusparttsbid + \"&tsbdomain=\" + Fusparttsdomain + \"&tsbvertical=\" + Fusparttsbvertical; window.frameElement.style.height = \"300px\"; window.frameElement.style.width = \"980px\"; <\/script> <scr"+"ipt src=\"http:\/\/fusion.bonniertidskrifter.se\/redirscrpt.js\"><\/script> ",
"currentSlot" : TS.pl_2.currentSlot,
"inProcess":false,
"name":"BT Fusion TS Desktop Mid Kvinna & Livsstil ",
"waterfallLevel" : 3,
"timeToLive":1,
"slotSpecific" : []
});
fillerCarousel2.fillerAds.push.apply(fillerCarousel2.fillerAds, tempA);
var tempA = [];
TS.log("pl_2  - Adding fillersupplier id: 22457, name: BT Fusion TS Desktop Inredning , placement: 2, waterfall: 4");
tempA.push({
"id" : 22457,
"sitePlacement" : 5284660,
"fillerSupplier" : 685,
"content" : "<scr"+"ipt> Fuspartnetwork = \"Tailsweep\"; Fusparttsbid = window.top.tsId; Fusparttsdomain = window.location.hostname; Fusparttsbvertical = \"Hem & Inredning\"; Fusadserver = \"fusion.bonniertidskrifter.se\"; Fusmediazone = \"bt.desktop.tailsweep.hem__inredning.ros\"; Fussinglespace = \"tailsweep_desktop_mid\"; Fusparameters = \"tsbnetwork=\" + Fuspartnetwork + \"&tsbid=\" + Fusparttsbid + \"&tsbdomain=\" + Fusparttsdomain + \"&tsbvertical=\" + Fusparttsbvertical; window.frameElement.style.height = \"300px\"; window.frameElement.style.width = \"980px\"; <\/script> <scr"+"ipt src=\"http:\/\/fusion.bonniertidskrifter.se\/redirscrpt.js\"><\/script> ",
"currentSlot" : TS.pl_2.currentSlot,
"inProcess":false,
"name":"BT Fusion TS Desktop Inredning ",
"waterfallLevel" : 4,
"timeToLive":1,
"slotSpecific" : []
});
fillerCarousel2.fillerAds.push.apply(fillerCarousel2.fillerAds, tempA);
var tempA = [];
TS.log("pl_2  - Adding fillersupplier id: 21168, name: Yieldoptimisers Desktop Panorama Shuffle Mat och Dryck, placement: 2, waterfall: 10");
tempA.push({
"id" : 21168,
"sitePlacement" : 5284660,
"fillerSupplier" : 469,
...
"currentSlot" : TS.pl_2.currentSlot,
"inProcess":false,
"name":"Yieldoptimisers Desktop Panorama Shuffle Mat och Dryck",
"waterfallLevel" : 10,
"timeToLive":1,
"slotSpecific" : []
});
...
TS.log("pl_2  - Fillers disabled");
}
/* Source: placementScript.js.vm */
(function(TS){
var i,
slotsProcessed;
if( TS.pl_2.currentSlot == G.ts_pl_2_idx ) {
slotsProcessed = TS.pl_2.currentSlot+1-G.ts_pl_2_idx;          TS.log("pl_2  - maxAds =  "+TS.pl_2.maxAds + ", slotsProcessed = " + slotsProcessed);
if(TS.isDebug() || true) {
TS.greyout = true;
if(TS.pl_2.currentSlot < TS.pl_2.maxAds+G.ts_pl_2_idx){
TS.greyout = false;
}
if(TS.isDebug()) {
TS.renderDebugAd(375, 250, 'Tailsweep Debug<br />midContent', TS.greyout);
}
else {
TS.log("pl_2  - starting");
if(TS.isMobileDevice()) {
TS.processPrioLevel(2, 0, slotsProcessed);
}
TS.log("pl_2  - no ads");
if(!TS.fillerAdsDisabled2()){
if( TS.isMobileDevice() ) {
if(fillerCarousel2.mobileFillerAds.length > 0)
{
TS.fillerCarousel2(fillerCarousel2);
}
}
else if (TS.isTablet()) {
if (fillerCarousel2.tabletFillerAds.length > 0)
...
if ("midContent" !== "17" || !TS.isMobileDevice()) {
var num = Math.floor((Math.random() * 100) + 1);
if(num < 95) {
TS.log("midContent" + " - Sending stats "+num);
TS.placementHit(2, TS.pl_2.currentSlot);
}
else {
TS.log("midContent" + " - Not sending stats "+num);
}
}
var placementSlot = {};
placementSlot.elementId = 'pl_' + 2 + '_' + TS.pl_2.currentSlot + '_inscreen';
placementSlot.placement = 2;
placementSlot.slot = TS.pl_2.currentSlot;
